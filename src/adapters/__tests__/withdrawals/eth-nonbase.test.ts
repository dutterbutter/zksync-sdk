...
describeForAdapters('adapters/withdrawals/routeEthNonBase', (kind, factory) => {
    it('builds a withdraw step targeting the base token system with gas helpers', async () => {
      ...
      const res = await ROUTES[kind].build({ token: L2_BASE_TOKEN_ADDRESS, amount, to } as any, ctx as any);
    ...
    it('preflight rejects when token mismatch or baseIsEth true', async () => {
      ...
      await expect(
        preflight({ token: L2_BASE_TOKEN_ADDRESS } as any, ctxEth as any),
      ).rejects.toThrow(/requires chain base ≠ ETH/i);
    });
    ...
    if (kind === 'ethers') {
      it('handles estimateGas failures by omitting gasLimit', async () => {
        ...
        const res = await ROUTES.ethers.build({ token: L2_BASE_TOKEN_ADDRESS, amount: 10n } as any, ctx as any);
