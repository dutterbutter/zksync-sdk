...
<ParamField name="p.token" type="Address" required>
  L1 token (use <code>0x…00</code> for ETH).
</ParamField>
...
<Tip>
  If <code>approvalsNeeded</code> is non-empty (ERC-20), <code>create</code> will include those
  steps automatically.
</Tip>
...
<Warning>
  If any step reverts, <code>create</code> throws a typed error. Prefer <code>tryCreate</code> to
  avoid exceptions.
</Warning>
...
<Tip>
  Prefer the <code>try*</code> variants if you want to avoid exceptions and work with result
  objects.
</Tip>
