...
// 1) Wait until included on L2 (adds l2ToL1Logs if available)
...
<Tip>
  Withdrawals are a two-step process: include on L2, then finalize on L1. You can wait for <code>ready</code> before calling <code>finalize</code>, or call <code>finalize</code> directly— it will throw if not yet ready.
</Tip>
...
<ParamField name="p.token" type="Address" required>
  L2 token address equivalent (ETH handled by the SDK).
</ParamField>
...
plan = {
  ...
  steps: [{ key, kind, tx: TransactionRequest }, ...]
}
*/
```
...
Prepare + **send** L2 steps. Returns a handle with the **L2 tx hash**.
...
<Warning>
  If any step reverts on L2, <code>create</code> throws a typed error. Prefer <code>tryCreate</code> if you want a result object.
</Warning>
